...
function openOrder() {
    ...
    // Показуємо popup замовлення
    document.getElementById("orderPopup").style.display = "flex";

    // Формуємо список замовлення
    let list = "";
    cart.forEach(i => {
    ...
    // ===== MOBILE: плавне згортання кошика =====
    if (window.innerWidth <= 700) {
        const cartBox = document.querySelector(".cart");
        if (cartBox.classList.contains("open")) {
            // зафіксувати поточну висоту перед transition
            cartBox.style.height = cartBox.offsetHeight + "px";
            cartBox.offsetHeight; // force reflow для запуску transition
            cartBox.style.transition = "height 0.3s ease"; // плавний перехід
            cartBox.classList.remove("open"); // прибираємо клас open

            // ставимо кінцеву висоту в наступному циклі
            setTimeout(() => {
                cartBox.style.height = "60px"; // кінцева висота для мобільного
            }, 10);
        }
    }
}
...
function closeOrder(){
    // Ховаємо вікно оформлення замовлення
    ...
    if(window.innerWidth <= 700){
        ...
        // Перемалювати вміст кошика, щоб кнопки та список відобразились
        render();

        // для мобільного кошика додаємо відновлення кнопок та суми
        cartBox.querySelector("p").style.display = "block";
        cartBox.querySelector(".buttons").style.display = "block";
        cartBox.querySelector("ul").style.display = "block";
    }
}
...
// щоб кнопки всередині працювали
if(
