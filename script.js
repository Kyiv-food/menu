...
function closeOrder() {
  ...
  if (window.innerWidth <= 700) {
    ...
    // Скидаємо клас open та inline-стилі
    cartBox.classList.remove("open");
    cartBox.style.transition = "";
    cartBox.style.height = "";

    // Сховати всі внутрішні елементи — залишаємо тільки кружечок
    ...
    // Перемалювати список замовлення, щоб цифра залишилась
    render();
  }
}
...
cartBox.addEventListener("click", function(e){
  ...
  const isOpen = cartBox.classList.toggle("open"); // додаємо/знімаємо клас
  ...
  if(isOpen){
    // Відкриваємо кошик: показуємо всі елементи
    ...
    // Закриваємо кошик: залишаємо тільки кружечок із цифрою
    if(cartList) cartList.style.display = "none";
    if(cartSum) cartSum.style.display = "none";
    if(cartButtons) cartButtons.style.display = "none";
  }
