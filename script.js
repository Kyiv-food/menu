...
const menu = {
  ...
  pasky:[
    {n:"Паска творожна",p:220,t:"550 г"},
    {n:"Паска творожна з цедрою лимона",p:220,t:"550 г"},
    {n:"Паска творожна з цедрою апельсина",p:220,t:"550 г"},
    {n:"Паска дріжджова",p:180,t:"550 г"},
    {n:"Паска творожна",p:100,t:"120 г"}
  ]
};
...
cards.forEach(card=>{
  ...
  menu[key].forEach(item=>{
    ...
    add.onclick = e => {
      e.stopPropagation();
      cart.push({ name:item.n, price:item.p, qty:count, t:item.t });
      sum += item.p * count;
      render();
      const cartBox = document.querySelector(".cart");
      cartBox.style.background="#d4ffd4";
      setTimeout(()=>{ cartBox.style.background="white"; },300);
   };
...
function closeOrder(){
  ...
  if(window.innerWidth <= 700){
    ...
    if(cartSum) cartSum.style.display="none";
...
function confirmOrder() {
  ...
  fetch(`https://api.telegram.org/bot${TOKEN}/sendMessage?chat_id=${CHAT_ID}&text=${encodedText}`);
...
window.addEventListener("load", function(){
  ...
  cartBox.addEventListener("click", function(e){
    ...
    const isOpen = cartBox.classList.toggle("open");
