...
const menu = {
  ...
  pasky:[
    ...
    {n:"Паска творожна",p:80,t:"120 г"},
    {n:"Паска творожна",p:100,t:"180 г"}
  ]
};
...
function confirmOrder() {
  ...
  // Формуємо детальний текст замовлення
  let text = `Замовлення\nТелефон: ${phone}\n\n`;
  cart.forEach(item => {
    text += `${item.name} (${item.t}) x${item.qty} - ${item.price * item.qty} грн\n`;
  });

  // Додаємо суму всього замовлення
