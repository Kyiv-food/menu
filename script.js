...
function closeOrder() {
  ...
  if (window.innerWidth <= 700) {
    ...
    // Прибираємо клас open, щоб кошик повернувся в міні-кружечок
    cartBox.classList.remove("open");
    
    // Забираємо inline стилі height/transition, щоб мобільний toggle працював
    cartBox.style.transition = "";
    cartBox.style.height = "";
    
    // Не ховаємо ul, p та кнопки — вони контролюються toggle при кліку
    // Тобто, при кліку на кружечок вони відкриються як раніше
  }
}
